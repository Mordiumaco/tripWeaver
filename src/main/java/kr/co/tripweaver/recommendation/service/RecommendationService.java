package kr.co.tripweaver.recommendation.service;
...
import kr.co.tripweaver.recommendation.model.BookVo;

 
@Service
public class RecommendationService {
    private static String clientID = "x6I0cwX3C7F7Z503W2Yl";
    private static String clientSecret = "BJ_3DiqhFj";
 
    //display ==> 몇개 출력
    //start==>몇번쨰부터 (item)
    public List<BookVo> searchBook(String keyword, int start, int display, String d_catg, String d_titl){
        List<BookVo> list = null;
        try {
            URL url;
            url = new URL("https://openapi.naver.com/v1/search/"
                    + "book_adv.xml?d_titl="
            		+ URLEncoder.encode(d_titl, "UTF-8")
                    + (d_catg != null ? "&display=" + display : "")
                    + (start !=0 ? "&start=" +start :"")
