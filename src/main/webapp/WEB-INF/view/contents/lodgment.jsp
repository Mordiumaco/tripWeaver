...
$(document).ready(function searchsearch(){
	...
	$('.searchUtill p strong').text("2569");
	$('#search').on('click', function () {
		areacode = $("select[name=areacode]").val();
		sigungucode = $("select[name=sigungucode]").val();

		
		if($('input:radio[name=stay]').is(':checked') == true){
			stay += "&";
			stay += $('input:radio[name="stay"]:checked').val();
			stay += "=1";
		}
		...
		var searchUrl ="http://api.visitkorea.or.kr/openapi/service/rest/KorService/searchStay?ServiceKey="+key + stay+"&areaCode="+areacode+"&sigunguCode="+sigungucode+"&listYN=Y&MobileOS=ETC&MobileApp=TourAPI3.0_Guide&arrange=A" ;
		
		totalCountSearch(searchUrl);
 ...
 $(document).ready(function(){
	$('.pg_wrap').on('click','.pg_page', function name() {
		...
		PagingUrl = "http://api.visitkorea.or.kr/openapi/service/rest/KorService/searchStay?ServiceKey="+key + stay+"&areaCode="+areacode+"&sigunguCode="+sigungucode+"&listYN=Y&MobileOS=ETC&MobileApp=TourAPI3.0_Guide&arrange=A&numOfRows=12&pageNo="+pageNum;

		totalCountSearch(PagingUrl);
	})
...
function totalCountSearch(searchUrl){
...
/* .sub_container ul li:nth-child(1) {display: none;}
.sub_container ul li:nth-child(2) {display: none;}
.sub_container ul li:nth-child(3) {display: none;}
.sub_container ul li:nth-child(4) {display: none;}
.sub_container ul li:nth-child(5) {display: none;}
.sub_container ul li:nth-child(6) {display: none;} */
...
<div class="sub_container">
	<h2 class="lodgment_title">지역별 관광정보</h2>
	<fieldset>
		 	...
		 	<table class="bbsSearch">
					...
					<tbody>
						<tr>
							...
							<td>
								...
								<input type="hidden" name="rlangtype" title="한국어" checked="checked" value="KOR" id="rlangType_KOR">								
				...
				<div id="selectStay">
					...
					<table class="bbsSearch" style="border:0px;">
						<caption>검색 순서 2 - 관광데이터 검색을 위한 지역 선택</caption>
						<colgroup>
							<col style="width:10%;">
				...
				<table class="bbsSearch">
					<caption>검색 순서 3  - 숙박 타입 선택 후 검색버튼을 클릭(엔터)하면 관광데이터 검색 결과를 확인할 수 있음</caption>
					<colgroup>
						<col style="width:10%;">
						<col style="width:90%;">
					</colgroup>
					<tbody>	
					    <tr>	
							<th  scope="row">숙박선택</th>
							<td>
								<input type="radio" name="stay" id="stay_full" value="" checked="checked"><label for="stay_full">전체</label>
							  
							    <input type="radio" name="stay" id="stay_goodStay" value="goodStay"><label for="stay_goodStay">굿스테이</label>
							  
							    <input type="radio" name="stay" id="stay_hanOk" value="hanOk"><label for="stay_hanOk">한옥</label>
							  
							    <input type="radio" name="stay" id="stay_benikia" value="benikia"><label for="stay_benikia">베니키아</label>
