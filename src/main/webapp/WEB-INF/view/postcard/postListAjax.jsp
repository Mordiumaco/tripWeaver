	...
	<c:choose>
		<c:when test="${pcl.pc_del == 'N'}">
			<ul class="postCard_con">
				<li>
					<div class="postli_l">
						<b> <img src="/file/read?mem_profile=${pcl.mem_profile}">
						</b> <span>${pcl.mem_nick}</span> <input type="hidden" id="pc_id"
							name="pc_id" value="${pcl.pc_id}">
					</div> <c:choose>
						<c:when test="${loginInfo.mem_id == pcl.mem_id}">
							<div class="postli_r">
								<a class="postcardUdate">수정</a> <a class="postcardDelete">삭제</a>
						...
						<ul class="slides">
							<c:forEach items="${pcl.attachmentList}" var="att">
								<c:choose>
									...
									<c:otherwise>
										<li><img src="/upload/postcard/${att.att_file_name}">
										</li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
				...
				<li>
					<div class="postli_l postli_l2">
						<c:choose>
							...
							<c:otherwise>
								...
								<input type="hidden" id="like_rel_art_id" name="like_rel_art_id"
									value="${pcl.pc_id}">
								<c:forEach varStatus="like" begin="0" end="${likeVo.size()}">
									<c:if test="${heart_loop}">
										<c:choose>
											<c:when
												test="${pcl.pc_id == likeVo[like.index].like_rel_art_id}">
					...
					<div class="postli_r">
						<i class="fa fa-share-alt fa-share-alt2 btn_share"
							aria-hidden="true"></i>
						<ul class="show_kakao" style="display: none;">
							<li>
								<div class="fb-share-button"
									data-href="http://192.168.203.53:8081/postCard/postCardList?u=${pcl.pc_id}&t=${pcl.pc_cnt}"
				...
				<li class="likeNumLi">좋아요 <b class="likeNum">${pcl.pc_like_count}</b>개
				</li>
				<li class="postli5">내용 <b class="more_btn">보기 +</b>
					<div class="postli5_con content">${pcl.pc_cnt}</div>
				</li>
				<li class="hashTaglink"><c:forEach items="${pcl.hashTagList}"
						var="htl">
						#<a>${htl}</a>
					</c:forEach></li>
				<li><fmt:formatDate value="${pcl.pc_date}"
						pattern="yyyy. MM. dd" /></li>
				<li class="postli5">댓글 <b class="more_btn">보기 +</b>
					<div class="postli5_con">
						<span class="Post_comment"> <c:choose>
								<c:when test="${pcl.commentList.size() != 0}">
									<c:forEach items="${pcl.commentList}" var="com"
										varStatus="comNum">
										<c:choose>
											<c:when test="${com.comt_del == 'N' }">
												<div class="comment_mam">
													<b>${com.mem_nick}</b> : <span>${com.comt_cnt}</span> <input
														type="hidden" id="mem_nick" name="mem_nick"
														value="${com.mem_nick}">
													<c:choose>
														<c:when test="${loginInfo.mem_id == com.mem_id}">
															<ul>
																<li><a class="bbtn_01 commentUpdate">수정</a> <input
																	type="hidden" id="comt_id" name="comt_id"
																	value="${com.comt_id}"></li>
																<li><a class="bbtn_02 commentDel">삭제</a> <input
																	type="hidden" id="comt_id" name="comt_id"
																	value="${com.comt_id}"></li>
				...
				<li><input id="comt_cnt" name="comt_cnt" type="text" placeholder="댓글달기...">
					<button id="com_btn" type="button">작성</button> &nbsp;&nbsp;&nbsp; <c:choose>
						<c:when test="${loginInfo.mem_id ne null }">
							<a href="javascript::"
								onclick="layer_open1('layer_1');return false;"
								class="declaration_btn"> <img
								src="/img/icon/declaration.png">
							</a>
						</c:when>
						<c:otherwise>
							<img src="/img/icon/declaration.png">
						</c:otherwise>
					</c:choose> <input type="hidden" id="comt_rel_art_id" name="comt_rel_art_id"
					value="${pcl.pc_id}"></li>
			</ul>
		</c:when>
	</c:choose>
